...
void main() {
  ...
  group('too many attempts tests', () {
    setUp(() {
      when(repository.isPinAuthenticationEnabled(userId: UserId('1'))).thenAnswer((_) => Future.value(true));
      when(repository.getPin(forUser: UserId('1'))).thenAnswer((_) async => PinHash(Pin('1').value));
    });

    test('unlocks with correct pin when below maxTries', () async {
      when(repository.getListOfFailedAttempts(userId: UserId('1'))).thenAnswer(
        (_) async => List.generate(4, (index) => DateTime.now()),
      );

      final result = await authenticator.unlockWithPin(pin: Pin('1'));
      verifyNever(repository.addFailedAttempt(any, forUser: UserId('1')));
      verify(repository.resetFailedAttempts(ofUser: UserId('1')));
      verify(repository.getListOfFailedAttempts(userId: UserId('1'))).called(1);
      expect(result, const Right(unit));
    });

    test('toomanyattemps is thrown when reaching maxtries with incorrect pin', () async {
      int tries = 4;
      when(repository.getListOfFailedAttempts(userId: UserId('1'))).thenAnswer(
        (_) async => List.generate(tries, (index) => DateTime.now()),
      );
      when(repository.addFailedAttempt(any, forUser: UserId('1'))).thenAnswer((realInvocation) {
        tries++;
        return Future.value(null);
      });

      final result = await authenticator.unlockWithPin(pin: Pin('2'));
      verify(repository.addFailedAttempt(any, forUser: UserId('1')));
      verify(repository.getListOfFailedAttempts(userId: UserId('1'))).called(2);
      verifyNever(repository.resetFailedAttempts(ofUser: UserId('1')));
      expect(result, const Left(LocalAuthFailure.tooManyAttempts));
    });

    test('attempt is thrown if maxtries was already reached', () async {
        when(repository.getListOfFailedAttempts(userId: UserId('1'))).thenAnswer(
              (_) async => List.generate(5, (index) => DateTime.now()),
        );
